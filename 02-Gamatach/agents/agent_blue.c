...
int potential_two(int player)
{
    int best_score = -1, best_stack = -1 ;

    for (int stack = 1 ; stack <= n_stacks ; stack++) {
        if (top[stack] > stack_cap) 
            continue;

        int level = top[stack] ;
        board[level][stack] = player ;
        top[stack]++ ;

        int eval_level = top[stack] - 1 ;

        int score = 0 ;
        int vertical = 1 + count_down(stack, eval_level, player) ;
        int horizontal = 1 + count_left(stack, eval_level, player) + count_right(stack, eval_level, player) ;
        int diag1 = 1 + count_down_left(stack, eval_level, player) + count_up_right(stack, eval_level, player) ;
        int diag2 = 1 + count_down_right(stack, eval_level, player) + count_up_left(stack, eval_level, player) ;

        if (vertical >= 2) score += vertical ;
        if (horizontal >= 2) score += horizontal ;
        if (diag1 >= 2) score += diag1 ;
        if (diag2 >= 2) score += diag2 ;

        if (vertical == 2) score += 1 ;
        if (horizontal == 2) score += 1 ;
        if (diag1 == 2) score += 1 ;
        if (diag2 == 2) score += 1 ;
    ...
    return best_stack != -1 ? best_stack : 0 ;
}
...
int main ()
{
	...
	if (choice = potential_two(this_player)) {
		printf("%c", 'A' + choice - 1) ;
		return EXIT_SUCCESS ;
	}
